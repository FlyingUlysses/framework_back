...
function showSeat(e,id,name){
	...
	$.get(url,function(res,status){
		...
		if(res != null ){
			var airportList = res.airport;
			var seat = res.seat;
			var airTempArray = [];
			if(airportList != null && airportList != undefined && airportList.length >0){
				$.each(airportList,function(i,item){
					airTempArray.push(item.airportId);
				});
			}
			airportSelect.val(airTempArray);
...
function reloadAirportAndElements(){
	var url = "/seat/listAllAirportAndElements";
	$.get(url,function(res,status){
		if(res != null){
			var airportList = res.airport;
			if(airportList != null && airportList != undefined && airportList.length >0){
