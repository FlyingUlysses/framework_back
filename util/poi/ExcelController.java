...
public class ExcelController {
    ...
    public void excel(HttpSession session,HttpServletResponse response,String start_time,String end_time,String type,String is_start) throws Exception {
        ...
        HashMap<String, ArrayList<FlightEntity>> map = flightService.reloadExcelTable(type,is_start, start_time, end_time, config.getSeatId());
        List<List<Object>>rows = new ArrayList<>();
        ArrayList<FlightEntity> flightList = new ArrayList<>();
        if(map.get("started") !=null) {
        	flightList.addAll(map.get("started"));
        }
        if(map.get("wait") != null) {
        	flightList.addAll(map.get("wait"));
        }
        ...
        data2.setName("测试测试");
        data2.setTitles(titles);
        data2.setRows(rows);
        dataList.add(data2);
        ExportExcelUtils.exportExcel(response,"航班情况.xlsx",dataList);
    }
	
	@RequestMapping(value="/excel/reloadTable")
	@ResponseBody
	public HashMap<String, ArrayList<FlightEntity>> reloadTable(HttpSession session,String start_time,String end_time,String type,String is_start) {
		LoginConfig config  = (LoginConfig) session.getAttribute(Constants.LOGIN_CONFIG);
		return flightService.reloadExcelTable(type,is_start, start_time, end_time, config.getSeatId());
	}
